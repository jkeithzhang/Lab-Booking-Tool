...
module.exports = function (options) {
    ...
    var FileInfo = require('./fileinfo')(
        ...
        }, _.pick(options, 'minFileSize', 'maxFileSize', 'acceptFileTypes', 'maxNumberOfFiles'))
    );
        ...
        form
            ...
            .on('file', function (name, file) {
                ...
                if (fs.existsSync(file.path)) {
                    ...
                    //tianwei
                    if (!fs.existsSync(options.decoreDir() + '/'))
                        mkdirp.sync(options.decoreDir() + '/');
                    if (!fs.existsSync(options.downloadDir() + '/'))
                        mkdirp.sync(options.downloadDir() + '/');
    ...
    UploadHandler.prototype.destroy = function () {
        ...
        //tianwei
        fs.unlink(options.decoreDir() + '/' + fileName + '.txt', function (ex) {
            // _.each(options.imageVersions, function (value, version) {
            //     fs.unlink(options.uploadDir() + '/' + version + '/' + fileName);
            // });
            self.emit('delete', fileName + '.txt');
            // self.callback({success: !ex});
        });

        fs.unlink(options.downloadDir() + '/' + fileName + '.cli.txt', function (ex) {
            // _.each(options.imageVersions, function (value, version) {
            //     fs.unlink(options.uploadDir() + '/' + version + '/' + fileName);
            // });
            self.emit('delete', fileName + '.cli.txt');
            // self.callback({success: !ex});
        });
    };
    ...
    UploadHandler.prototype.initUrls = function (fileInfo) {
        ...
        //tianwei
        fileInfo.setDecoreInfo(options.decoreAppDir(), options.uploadDir(), options.decoreDir(), baseUrl + options.decoreUrl(), options.downloadDir());
        _.each(options.imageVersions, function (value, version) {
